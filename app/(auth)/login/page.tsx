    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <div className="relative flex flex-col items-center justify-center pt-20 pb-16 px-6 overflow-hidden">
        ...
        <div className="relative flex flex-col items-center gap-3">
          ...
          <p style={{ color: 'var(--muted)', fontSize: '14px' }}>Tu billetera digital</p>
        </div>
      </div>

      {/* Form sheet */}
      <div className="flex-1 rounded-t-[28px] px-6 pt-8 pb-8" style={{ background: 'var(--surface)' }}>
        <h2 className="text-xl font-semibold mb-6" style={{ color: 'var(--text)' }}>Iniciá sesión</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Email</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Contraseña</label>
            <div className="relative">
              <input type={showPass ? 'text' : 'password'} required autoComplete="current-password"
                ...
                style={{
                  background: 'var(--surface-2)', border: '1px solid var(--border)',
                  borderRadius: '14px', padding: '14px 48px 14px 16px', fontSize: '15px',
                  color: 'var(--text)', outline: 'none', transition: 'border-color 0.2s',
                  width: '100%'
                }}
                onFocus={e => e.target.style.borderColor = 'var(--primary)'}
        ...
        <p className="text-center mt-6" style={{ fontSize: '14px', color: 'var(--muted)' }}>
          ¿No tenés cuenta?{' '}
          <Link href="/register" style={{ color: 'var(--primary-2)', fontWeight: 500 }}>
            Registrate gratis
          </Link>
        </p>
      </div>
